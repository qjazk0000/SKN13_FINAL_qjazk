...
export const authService = {
  ...
  async logout() {
    try {
      const res = await api.post("/auth/logout/"); // ⬅️ '/api' 제거
      return res.data;
    } catch (err) {
  ...
  async checkAuthStatus() {
    try {
      ...
      const res = await api.get("/auth/profile/"); // ⬅️ '/api' 제거
  ...
  async refreshToken() {
    try {
      ...
      const res = await api.post("/auth/refresh/", { refresh: refreshToken }); // ⬅️ '/api' 제거
      if (res.data?.success && res.data?.data) {
  ...
  async updateUserProfile(userData) {
    try {
      const res = await api.put("/auth/profile/", userData); // ⬅️ '/api' 제거
      if (res.data?.success) return res.data.data;
  ...
  async changePassword(passwordData) {
    try {
      // ⬇️ '/api' 제거 (백엔드 라우트가 /user/password-change/)
